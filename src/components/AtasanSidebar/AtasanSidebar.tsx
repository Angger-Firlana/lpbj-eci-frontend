...
import styles from './AtasanSidebar.module.css';

interface AtasanSidebarProps {
  onLogout?: () => void;
  inboxCount?: number;
}

const AtasanSidebar: FC<AtasanSidebarProps> = ({ onLogout, inboxCount = 0 }) => {
  const location = useLocation();
  const currentPath = location.pathname;

  const isActive = (path: string) => {
    if (path === '/atasan/inbox') {
      return currentPath.startsWith('/atasan/inbox');
    }
    if (path === '/atasan/history') {
      return currentPath.startsWith('/atasan/history');
    }
  ...
  return (
    <aside className={styles.sidebar}>
      ...
      <nav className={styles.nav}>
        <Link
          to="/atasan/dashboard"
          className={`${styles.menuItem} ${isActive('/atasan/dashboard') ? styles.active : ''}`}
        >
        ...
        <Link
          to="/atasan/inbox"
          className={`${styles.menuItem} ${isActive('/atasan/inbox') ? styles.active : ''}`}
        >
          <span className={styles.menuIcon}>
          ...
          <span className={styles.menuLabel}>Inbox</span>
          {inboxCount > 0 && (
        ...
        <Link
          to="/atasan/history"
          className={`${styles.menuItem} ${isActive('/atasan/history') ? styles.active : ''}`}
        >
          <span className={styles.menuIcon}>
